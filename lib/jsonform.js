...
var fileDisplayTemplate = '<div class="_jsonform-preview">' +
  ...
  '<% } %>' +
  '</div>' +
  '<a href="#" class="btn btn-danger _jsonform-delete"><i class="glyphicon glyphicon-remove" title="Remove"></i></a> ';
...
jsonform.elementTypes = {
  ...
  'array': {
    'template': '<div id="<%= id %>"><ul class="_jsonform-array-ul" style="list-style-type:none;"><%= children %></ul>' +
      '<span class="_jsonform-array-buttons">' +
        '<a href="#" class="btn btn-success _jsonform-array-addmore"><i class="fa fa-plus-circle" title="Add new"></i></a> ' +
        '<a href="#" class="btn btn-danger _jsonform-array-deletelast"><i class="fa fa-minus-circle" title="Delete last"></i></a>' +
      '</span>' +
      '</div>',
    'fieldtemplate': true,
    'array': true,
  ...
  'tabarray': {
    'template': '<div id="<%= id %>"><div class="tabbable tabs-left">' +
      ...
      '</div>' +
      '<a href="#" class="btn btn-success _jsonform-array-addmore"><i class="fa fa-plus-circle" title="Add new"></i></a> ' +
      '<a href="#" class="btn btn-danger _jsonform-array-deleteitem"><i class="fa fa-minus-circle" title="Delete item"></i></a></div>',
    'fieldtemplate': true,
    'array': true,
    'childTemplate': function (inner) {
    ...
    'onBeforeRender': function (data, node) {
      ...
      _.each(node.children, function (child, idx) {
        ...
        tabs += '<li data-idx="' + idx + '"' +
          ((idx === 0) ? ' class="active"' : '') +
          '><a class="draggable tab nav-link" data-toggle="tab">' +
          escapeHTML(title) +
          '</a></li>';
      });
      data.tabs = tabs;
   ...
   */
  'question': {
    'template': '<div id="<%= node.id %>"><% _.each(node.options, function(key, val) { %><label class="radio<%= (node.formElement.optionsType === "radiobuttons") ? " btn btn-default" : "" %><%= ((key instanceof Object && key.htmlClass) ? " " + key.htmlClass : "") %>"><input type="radio" <% if (node.formElement.optionsType === "radiobuttons") { %> style="position:absolute;left:-9999px;" <% } %>name="<%= node.id %>" value="<%= val %>"<%= (node.disabled? " disabled" : "")%>/><span><%= (key instanceof Object ? key.title : key) %></span></label> <% }); %></div>',
    'fieldtemplate': true,
    'onInsert': function (evt, node) {
      var activeClass = 'active';
      var elt = node.formElement || {};
